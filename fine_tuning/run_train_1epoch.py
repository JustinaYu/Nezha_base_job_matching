...
def run_1epoch_train(args, accelerator):
    eval_dataset = torch.load(config['train_data_dir'] / args.cached_train_dataset)
    ...
    #         labels_res.extend(gathered_label_ids)
    #         predictions_res.extend(gathered_idx)
    ...
    # results = zip(labels_res, predictions_res)
    # if accelerator.is_main_process:
    #     res_path = config['train_data_dir'] / 'fl-adjust-predict.csv'
    #     with open(res_path, 'w', newline='') as csvfile:
    #         writer = csv.writer(csvfile)
    #         writer.writerow(['Label', 'Predition'])
    #         for item1, item2 in results:
    #             writer.writerow([int(item1.item()), int(item2.item())])
...
def main():
    ...
    # checkpoint-second-3epoch/900.0-ckpt-20241124_120949.pt
    # checkpoints/400.0-ckpt-20241125_090940.pt
    parser.add_argument('--load_model_ckpt', type=str,
                        default="fine_tuning/finetuning_model/checkpoints/200.0-ckpt-20241125_094536.pt",
    ...
    if args.do_eval:  # conduct dataset generation
        run_1epoch_train(args, accelerator)
