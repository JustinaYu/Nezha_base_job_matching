...
class FineTuningProcessor(object):
    """Base class for data converters for sequence classification data sets."""

    def __init__(self, vocab_path, document_file, position_file, args):
    ...
    def create_examples_from_document(self, tokens, resumeid, max_seq_length):
        ...
        label = int(position_data[resumeid])-1
        # 截断
        ...
        instance = InputExample(input_ids=input_ids, attention_mask=attention_mask, label=label)
        return instance
    ...
    def create_dataset(self, examples: List[InputExample]):
        ...
        all_labels = torch.tensor([f.label for f in examples], dtype=torch.float)
